...
namespace Reallukee.GWC
{
    public sealed class Window : IDisposable
    {
                ...
                if (window.IsHandleCreated)
                {
                    lock (renderLock)
                    {
                        try
                        {
                            ...
                            {
                                while (canvas.Buffer.TryDequeue(out IGRenderable renderable))
                                {
                                    renderable.Render(g);
        ...
        {
            return this.canvas.ClearArea(x, y, width, height);
        }

        public bool ClearF()
        {
            return this.canvas.ClearF();
        }

        public bool ClearAreaF(float x, float y, float width, float height)
        {
            return this.canvas.ClearAreaF(x, y, width, height);
        }
